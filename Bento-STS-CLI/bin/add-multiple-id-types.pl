...
sub count_nanoid6 {
    ...
    print "... starting counting a.nanoid6" if $VERBOSE > 4;

    my $count;

    my $stream = $CXN->run_query(
      "MATCH (a) WHERE a.nanoid6 IS NOT NULL RETURN count(a);",
      {} # parameter hash required
    );
    my @names = $stream->field_names;
    while ( my @row = $stream->fetch_next ) {
      print "\tnumber of a.nanoid6: $row[0]\n" if $VERBOSE > 2;
      $count = $row[0];
    };

    print "... done counting a.nanoid6 \n" if $VERBOSE > 4;
    return $count;
}
...
sub add_id_to_file {
    ...
    for my $f (@files) {
        ...
        while (my $line = <$fh>) {
           chomp $line;
            my $nanoid = Nanoid::generate('size'=>10, 'alphabet'=>'abcdefghijkmnopqrstuvwxyzABCDEFGHJKMNPQRSTUVWXYZ0123456789');
...
sub make_nanoid {
    my $size = shift // 12;
    my $alphabet = shift // 'abcdefghijkmnopqrstuvwxyzACEFGHJKMNPQRTUVWXYZ023456789'; 
...
sub generate_ids {
    ...
    my $nanoid6 = make_nanoid(6); 
    my $nanoid8 = make_nanoid(8); 
    my $nanoid10 = make_nanoid(10); 

    my $uuid = make_uuid();

    my $halfuuid = make_halfuuid($uuid);
    
    my $hashid = make_hashid($halfuuid);
...
sub update_one_thing {
    ...
    if ($existing_id_count == 0){
        my $counter = 0;
        my $neo4jid = get_a_thing_neo4jid('nanoid6'); 
...
sub update_all_things {
    ...
    my $counter = 0;
    count_nanoid6();
    my $neo4j_id_to_update = get_a_thing_neo4jid('nanoid6'); 
    while ($neo4j_id_to_update) {
        ...
        $counter++;
        $neo4j_id_to_update = get_a_thing_neo4jid('nanoid6');
...
The error `Can't call method "field_names"` generally means the script could not connect to the database.
...
```
Can't call method "field_names" on an undefined value at 22-add-multiple-id-types.pl line 3    15.
```
...
TODO: 
* add tests
* credentials NEO4J_MDB_USER, NEO4J_MDB_PASS are in the environment
