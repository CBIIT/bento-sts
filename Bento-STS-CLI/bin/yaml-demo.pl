...
# make a simple yaml 
#-----------------------------------------------------------------------------
my $usage = '$> perl yaml-demo.pl [ -mdb DEV|STAGE|PROD ] [ -dev P|Y|D ] [ -v | -verbose] [ -debug=# ]';
...
sub run {
    ...
    ## example 3
    #print "\n\nExample 3\n\n";
    #Bento::STS::CLI::get_icdc_nodes($bolt_url);

    my $icdc_model = Bento::STS::CLI::get_icdc_model($bolt_url);
    my $yaml = Bento::STS::CLI::Yaml::get_model_yaml($icdc_model);
    print "$yaml\n";
